...
@section('content')
    <div>
        <div class="account-card">
            <div class="title">Обліковий запис <div class="account-ava"><img src="{{ asset('icons/user.svg') }}"></div></div>
            <div>#{{ $user->id }}</div>
            <div>Ім'я: <span>{{ $user->name }}</span></div>
            <div>Логін: <span>{{ $user->email }}</span></div>
            <div>Створено: <span>{{ $user->created_at }}</span></div>
            <div>
                <a href="{{ route('logout') }}">
                    <div class="box-icon">
